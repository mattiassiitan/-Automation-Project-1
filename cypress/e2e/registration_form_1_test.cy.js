...
describe('This is first test suite, Mattias Siitan', () => {
//14/06/2023
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Mattias')
        cy.get('#lastName').type('Siitan')
        cy.get('input[name="password"]').type('Klabanaator')
        cy.get('[name="confirm"]').type('Klabanaator')
    ...
    */
    ...
    it('User cannot submit data when password and/or confirmation password is absent', () => {
        ...
        // Assert that submit button is not enabled and that successful message is not visible
        cy.get('#username').type('Klaabu')
        cy.get('#phoneNumber').type('55653563')
